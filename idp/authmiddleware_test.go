...
func TestGetRequestWithFalseAuthorizationType_RedirectsToIdp(t *testing.T) {
	...
	idp.HandleAuth(nil, nil, log, log)(&handlerSpy).ServeHTTP(responseSpy, req)
...
func TestRequestWithBearerAuthorization_PopulatesContextWithPrincipalAndAuthsession(t *testing.T) {
	...
	handlerSpy := handlerSpy{}
	idpStub := newIdpStub(map[string]scim.Principal{authSessionId: principal})
	defer idpStub.Close()

	idp.HandleAuth(returnFromCtx(idpStub.URL), returnFromCtx("1"), log, log)(&handlerSpy).ServeHTTP(httptest.NewRecorder(), req)
...
func TestRequestWithBadUrlEncodedAuthSessionIdCookie_ReturnsStatus500(t *testing.T) {
	...
	idp.HandleAuth(returnFromCtx(idpStub.URL), returnFromCtx("1"), log, log)(&handlerSpy).ServeHTTP(spy, req)
...
func TestGetSystemBaseUriFromCtxReturnsError_ReturnsStatus500(t *testing.T) {
	...
	idp.HandleAuth(func(ctx context.Context) (string, error) { return "", errors.New("any error") }, returnFromCtx("1"), log, log)(&handlerSpy).ServeHTTP(spy, req)
...
func TestGetTenantIdFromCtxReturnsError_ReturnsStatus500(t *testing.T) {
	...
	idp.HandleAuth(returnFromCtx(idpStub.URL), func(ctx context.Context) (string, error) { return "", errors.New("any error") }, log, log)(&handlerSpy).ServeHTTP(spy, req)
...
func TestUserIsCachedForDifferentTenant_CallsIdp(t *testing.T) {
	...
	idpStub1 := newIdpStub(map[string]scim.Principal{authSessionId: principalT1})
	defer idpStub1.Close()
	handlerSpy1 := handlerSpy{}
	idp.HandleAuth(returnFromCtx(idpStub1.URL), returnFromCtx("1"), log, log)(&handlerSpy1).ServeHTTP(httptest.NewRecorder(), reqTenant1)
	...
	idpStub2 := newIdpStub(map[string]scim.Principal{authSessionId: principalT2})
	defer idpStub2.Close()
	handlerSpy2 := handlerSpy{}
	idp.HandleAuth(returnFromCtx(idpStub2.URL), returnFromCtx("2"), log, log)(&handlerSpy2).ServeHTTP(httptest.NewRecorder(), reqTenant2)
...
func newIdpStub(principals map[string]scim.Principal) *httptest.Server {
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/identityprovider/validate" {
			...
			w.Header().Set("Cache-Control", "max-age=1800, private")
			w.Header().Set("Content-Type", "application/hal+json; charset=utf-8")
			_ = json.NewEncoder(w).Encode(principals[authToken])
			return
		}
